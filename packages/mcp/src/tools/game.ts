...
export function registerGameTools(server: McpUnoServer): void {
  ...
  mcp.tool('pass', '过牌（无牌可出时）', () => wrapTool(() => server.getClient().pass()));

  mcp.tool('call_uno', '喊 UNO（手牌剩 1 张时）', () => wrapTool(() => server.getClient().callUno()));

  mcp.tool('catch_uno', '抓别人没喊 UNO', { targetPlayerId: z.string().describe('目标玩家 ID') }, args =>
    wrapTool(() => server.getClient().catchUno({ targetPlayerId: args.targetPlayerId })),
  );

  mcp.tool('challenge', '挑战对手的 Wild Draw Four', () => wrapTool(() => server.getClient().challenge()));

  mcp.tool('accept', '接受罚牌', () => wrapTool(() => server.getClient().accept()));
  ...
  mcp.tool('vote_next_round', '投票开始下一轮', () => wrapTool(() => server.getClient().voteNextRound()));

  mcp.tool('back_to_room', '游戏结束后返回等待房间（仅房主）', () => wrapTool(() => server.getClient().backToRoom()));
}
