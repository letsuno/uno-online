...
export const finishRestrictions: HouseRulePlugin = {
  ...
  preCheck: (state: GameState, action: GameAction, ctx: RuleContext): PreCheckResult => {
    ...
    if (
      isLast &&
      ((hr.noWildFinish && ctx.isWildCard(card)) || (hr.noFunctionCardFinish && ctx.isFunctionCard(card)))
    ) {
      ...
      if (!penalizedPlayer) {
        throw new Error(`Finish penalty player is missing: ${action.playerId}`);
      }
      if (penalizedPlayer.hand.length === player.hand.length) {
