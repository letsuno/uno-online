...
import type { Card, CommittedGameAction } from '@uno-online/shared';
import { useGameStore } from '../stores/game-store';
import type { PlayerInfo } from '../stores/game-store';
import { useGameLogStore } from '../stores/game-log-store';

function getActionKey(action: CommittedGameAction, players: PlayerInfo[]): string {
  switch (action.type) {
    case 'PLAY_CARD':
  ...
  useEffect(() => {
    ...
    if (lastAction.type === 'PLAY_CARD') {
      ...
      const typeMap: Record<
        Card['type'],
        'play_number' | 'play_skip' | 'play_reverse' | 'play_draw_two' | 'play_wild' | 'play_wild_draw_four'
      > = {
        number: 'play_number',
        skip: 'play_skip',
      ...
      addLogEntry({
        ...
        playerName: lastAction.catcherName,
      ...
      const player = findPlayer(lastAction.playerId);
      if (!player) return;
      const penaltyPlayer = findPlayer(lastAction.penaltyPlayerId);
      if (!penaltyPlayer) return;
      addLogEntry({
        type: 'challenge',
        playerId: lastAction.playerId,
        playerName: player.name,
        extra: `${lastAction.succeeded ? '成功' : '失败'}，${penaltyPlayer.name} 罚摸 ${lastAction.penaltyCount} 张`,
      });
