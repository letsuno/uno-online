...
export default function GameActions({
  ...
  const ownHandCount = me?.handCount ?? 0;
  ...
  const canPassAfterDraw =
    pendingPenaltyDraws === 0 &&
    drawStack === 0 &&
    hasDrawnThisTurn &&
    (!mustDrawUntilPlayable || playableIds.size > 0);
  ...
  const stuckAtHandLimit =
    handLimit !== null &&
    ownHandCount >= handLimit &&
    playableIds.size === 0 &&
    pendingPenaltyDraws === 0 &&
    drawStack === 0;
  ...
  return (
    <div className="relative z-actions flex justify-center gap-2.5 py-2 pointer-events-auto">
      {canCallUno && (
        <Button variant="primary" onClick={withCooldown(onCallUno)} disabled={cooldown} sound="action">
          喊 UNO!
        </Button>
      )}
      {catchTargets.map(t => (
      ...
      {phase === 'challenging' && pendingDrawPlayerId === userId && (
        <>
          {!noChallengeWD4 && (
            <Button variant="danger" onClick={onChallenge} sound="action">
              质疑!
            </Button>
          )}
          <Button variant="secondary" onClick={onAccept} sound="action">
            接受
          </Button>
        </>
      )}
      {isMyTurn && canPass && phase === 'playing' && (
        <Button variant="secondary" onClick={onPass} sound="click">
          跳过
        </Button>
      )}
