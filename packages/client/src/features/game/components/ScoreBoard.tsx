...
export default function ScoreBoard({
  ...
  const { queue: pendingJoinQueue, queued: spectatorQueued, toggle: toggleSpectatorQueue } = useSpectatorQueue();
  ...
  useEffect(() => {
    const endAt = roundEndAt ?? gameOverAt;
    const initial = endAt ? Math.max(0, START_COOLDOWN_S - Math.floor((serverNow() - endAt) / 1000)) : START_COOLDOWN_S;
  ...
  const allAgreed = vote !== null && vote.votes >= vote.required;
  const cooldownActive = startCooldown > 0;
  const noHumanPlayers = vote?.required === 0;
        ...
        : isHost
          ? noHumanPlayers
            ...
            : hasVoted
              ? allAgreed
                ? '开始下一轮'
                : `等待同意 (${vote.votes}/${vote.required})`
              : `同意继续 (${vote.votes}/${vote.required})`
          : hasVoted
            ? allAgreed
              ? '等待房主开始'
              : `已同意 (${vote.votes}/${vote.required})`
            : `同意继续 (${vote.votes}/${vote.required})`;
  const isNextRoundDisabled =
    !isGameOver &&
    (!vote ||
      (isSpectatorOwner && !spectatorQueued) ||
      (isHost ? (noHumanPlayers ? cooldownActive : hasVoted && (!allAgreed || cooldownActive)) : hasVoted));
      ...
      <div className="glass-panel w-[460px] max-w-[94vw] px-7 py-6 max-md:px-5 text-center">
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isWinner ? 'bg-primary/[0.08] border-primary/30' : 'bg-white/[0.03] border-transparent',
              ...
              >
                ...
                <div className="relative shrink-0">
                  ...
                  {isWinner && (
                    <Crown
                      size={13}
                      className="absolute -top-2 -right-1.5 rotate-[18deg] text-primary drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)]"
                    />
                  )}
                </div>
                ...
                <div className="flex-1 min-w-0 flex items-center gap-1.5">
                  <span className={cn('text-sm truncate', isWinner ? 'text-primary font-bold' : 'text-foreground')}>
                    {p.name}
                  </span>
                  {isSelf && (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  {!isGameOver && (
                    <>
                      {ready ? (
                        <span
                          className="w-4.5 h-4.5 rounded-full bg-uno-green/15 flex items-center justify-center"
                          title="已同意下一轮"
                        >
                          <Check size={11} className="text-uno-green" />
                        </span>
                      ) : (
                      ...
                      {isHost && !isSelf && (
                        <button
                          ...
                          className="text-destructive/60 hover:text-destructive cursor-pointer bg-transparent border-none transition-colors"
                          title={p.isBot ? '移除机器人' : '移至观战席'}
                        >
                          <UserX size={13} />
                        </button>
                      )}
                  ...
                  <span
                    className={cn(
                      'min-w-[36px] text-right font-black tabular-nums',
                      isWinner ? 'text-primary text-base' : 'text-foreground text-sm',
                    )}
                  >
                    {p.score}
        ...
        {!isGameOver && pendingJoinQueue.length > 0 && (
          <p className="mb-2 text-xs text-accent flex items-center justify-center gap-1">
            <UserPlus size={12} /> {pendingJoinQueue.map(entry => entry.nickname).join('、')} 将在下一轮加入
          </p>
        )}
        {!isGameOver && !isSpectator && (
          <p className="mb-3 text-xs text-muted-foreground">
            {!vote
              ...
              : allAgreed
                ? '所有玩家已同意，等待房主开始下一轮'
                : `已有 ${vote.votes}/${vote.required} 人同意继续下一轮`}
          </p>
        )}
        {cooldownActive && (
        ...
        {isSpectator ? (
          <div className="flex gap-3 justify-center flex-wrap">
            {!isGameOver && isHost && (
              <Button variant="primary" onClick={onPlayAgain} disabled={isNextRoundDisabled} sound="ready">
                {nextRoundButtonText}
              </Button>
            )}
            {!isGameOver &&
              (() => {
                ...
                const Icon = spectatorQueued
                  ? locked
                    ? Check
                    : X
                  : queueBlocked && isSpectatorOwner
                    ? UserX
                    : UserPlus;
                ...
                return (
                  <Button
                    variant="secondary"
                    onClick={locked || queueBlocked ? undefined : toggleSpectatorQueue}
                    disabled={locked || queueBlocked}
                    sound={spectatorQueued ? 'click' : 'ready'}
                  >
                    <Icon size={14} className="inline align-middle mr-1" />
                    {label}
                  </Button>
                );
              })()}
            {isGameOver && isHost && (
              <Button variant="primary" onClick={onBackToRoom} disabled={cooldownActive} sound="ready">
                返回房间
              </Button>
            )}
            {isGameOver && !isHost && (
            ...
            <Button variant="secondary" onClick={onBackToLobby} sound="click" disabled={leaveCountdown > 0}>
              {leaveCountdown > 0 ? `返回大厅 (${leaveCountdown}s)` : '退出房间'}
            </Button>
          </div>
        ) : (
          <div className="flex gap-3 justify-center flex-wrap">
            ...
            {!isGameOver && (
              <Button variant="secondary" onClick={onLeaveToSpectate} sound="click">
                <Eye size={14} className="inline align-middle mr-1" />
                进入观战席
              </Button>
            )}
            <Button variant="secondary" onClick={onBackToLobby} sound="click" disabled={leaveCountdown > 0}>
              {leaveCountdown > 0 ? `返回大厅并托管 (${leaveCountdown}s)` : '返回大厅并托管'}
            </Button>
          </div>
        )}
      </div>
