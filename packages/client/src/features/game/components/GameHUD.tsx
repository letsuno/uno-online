...
function HudIconButton({ onClick, active = false, disabled = false, danger = false, title, children }: {
  onClick: () => void;
  active?: boolean;
  disabled?: boolean;
  danger?: boolean;
...
export default function GameHUD({ roomCode, mode, onOpenHotkeys, onOpenMenu, onOpenInfo }: GameHUDProps) {
  ...
  const handleDissolve = async () => {
    if (!(await showConfirm({ title: '解散房间', message: '确定要解散房间吗？所有玩家将被踢出。', confirmText: '解散', variant: 'danger' }))) return;
    ...
    <div className="flex items-center gap-3 px-4 h-11 bg-background/60 backdrop-blur-md text-caption z-topbar">
      ...
      <FitScaler mode="width" align="start" origin="left center" className="flex-1 min-w-0 h-8 pointer-events-none">
        <div className="flex items-center gap-3 w-fit pointer-events-auto">
          <span className="font-bold text-primary font-game whitespace-nowrap"><Spade size={18} className="inline align-middle" /> UNO Online</span>
      ...
      <FitScaler mode="width" align="end" origin="right center" className="flex-1 min-w-0 h-8 pointer-events-none">
        <div className="flex items-center gap-3 w-fit pointer-events-auto">
          <HudIconButton onClick={onOpenHotkeys} title="快捷键设置"><Keyboard size={16} /></HudIconButton>
          <HudIconButton onClick={toggleInfoDrawer} title="游戏信息 (H)"><HelpCircle size={16} /></HudIconButton>
          <HudIconButton onClick={handleToggleAutopilot} disabled={autopilotCooldown} active={myAutopilot} title={autopilotCooldown ? '操作冷却中...' : myAutopilot ? '关闭自动托管' : '开启自动托管'}><Bot size={16} /></HudIconButton>
          <HudIconButton onClick={toggleColorBlind} active={colorBlindMode} title={colorBlindMode ? '关闭色盲模式' : '开启色盲模式'}><Eye size={16} /></HudIconButton>
          <HudIconButton onClick={toggleBgm} active={bgmEnabled} title={bgmEnabled ? '关闭背景音乐' : '开启背景音乐'}><Music size={16} /></HudIconButton>
          <HudIconButton onClick={toggleSound} active={soundEnabled} title={soundEnabled ? '关闭音效' : '开启音效'}>{soundEnabled ? <Volume2 size={16} /> : <VolumeX size={16} />}</HudIconButton>
          <TurnTimer />
          <HudIconButton onClick={handleLeave} danger title={isHost ? '离开并转让房主' : '退出对局'}>{isHost ? <DoorOpen size={16} /> : <LogOut size={16} />}</HudIconButton>
          {isHost && (
            <HudIconButton onClick={handleDissolve} danger title="解散房间"><Trash2 size={16} /></HudIconButton>
          )}
        </div>
      </FitScaler>
    </div>
