    ...
    <div className="flex flex-col gap-1">
      {players.map((p, i) => {
        ...
        return (
          <div
            key={p.id}
            className={cn('flex items-center gap-2.5 rounded-xl px-2.5 py-2', isCurrent && 'bg-primary/10')}
          >
            <div
              className={cn(
            ...
            >
              {p.isBot ? (
                <BotAvatarIcon difficulty={p.botConfig?.difficulty} size={14} />
              ) : p.avatarUrl ? (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[11px] text-muted-foreground">
                {p.score} 分{p.roundWins ? ` · 🏆${p.roundWins}` : ''}
                {p.calledUno ? ' · UNO!' : ''}
                {p.autopilot ? ' · 托管' : ''}
              </div>
            </div>
            <span className="text-sm font-bold text-muted-foreground tabular-nums shrink-0">{p.handCount} 张</span>
          </div>
        );
      })}
