...
function sw(...beats: [N, N][]): N[] {
  return beats.flatMap(([a, b]) => [a, _, b]);
}
// 走路低音：每小节四个四分音符
function walk(...bars: [N, N, N, N][]): N[] {
  return bars.flatMap(([a, b, c, d]) => [a, _, _, b, _, _, c, _, _, d, _, _]);
}
// 伴奏切分：只在第 2、4 拍的反拍上点和弦内音
function comp(...bars: [N, N][]): N[] {
  return bars.flatMap(([a, b]) => [_, _, _, _, _, a, _, _, _, _, _, b]);
}
...
const song: Song = {
  name: 'Cardsharp',
  meta: { author: 'Claude Fable 5', key: 'D 小调', style: '摇摆爵士', wave: '方波' },
  bpm: 112,
  stepsPerBeat: 3,
  tones: [
    { wave: 'square', gain: 0.09, dur: 0.13, notes: join(mA, mB, mA, mC, mD, mA, mB, mE) },
    { wave: 'square', gain: 0.04, dur: 0.1, notes: join(hA, hB, hA, hC, hD, hA, hB, hE) },
    { wave: 'triangle', gain: 0.2, dur: 0.16, notes: join(bA, bB, bA, bC, bD, bA, bB, bE) },
  ],
  kick: {
    gain: 0.13,
    hits: [
