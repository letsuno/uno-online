...
function oom(...roots: N[]): N[] {
  return roots.flatMap(r => [r, _, _, _, _, _]);
}
function pah(...bars: [N, N][]): N[] {
  return bars.flatMap(([a, b]) => [_, _, a, _, b, _]);
}
...
const song: Song = {
  name: 'Carousel',
  meta: { author: 'Claude Fable 5', key: 'G 大调', style: '旋转木马圆舞曲', wave: '三角波' },
  bpm: 96,
  stepsPerBeat: 2,
  tones: [
    { wave: 'triangle', gain: 0.13, dur: 0.28, notes: join(mA, mB, mA, mC, mA, mE) },
    { wave: 'square', gain: 0.035, dur: 0.14, notes: join(hA, hB, hA, hC, hA, hE) },
    { wave: 'triangle', gain: 0.16, dur: 0.3, notes: join(bA, bB, bA, bC, bA, bE) },
  ],
  kick: { gain: 0.07, hits: rep([T, F, F, F, F, F], 48) },
  snare: { gain: 0.0, hits: rep([F, F, F, F, F, F], 48) },
