...
export class RoomManager {
  ...
  async createRoom(ownerId: string, ownerName: string, settings: RoomSettings = { turnTimeLimit: 30, targetScore: 500, houseRules: DEFAULT_HOUSE_RULES }, avatarUrl?: string | null): Promise<string> {
    ...
    await addPlayerToRoom(this.redis, code, { userId: ownerId, username: ownerName, avatarUrl });
    return code;
  }

  async joinRoom(roomCode: string, userId: string, username: string, avatarUrl?: string | null): Promise<void> {
    ...
    await addPlayerToRoom(this.redis, roomCode, { userId, username, avatarUrl });
  }
