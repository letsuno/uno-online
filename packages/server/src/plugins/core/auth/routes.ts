...
function registerDevRoutes(fastify: FastifyInstance, ctx: PluginContext) {
  ...
  fastify.get('/auth/me', { preHandler: authPreHandler(config.jwtSecret) }, async request => {
    ...
    return { id: p.userId, username: p.username, nickname: p.nickname, avatarUrl: p.avatarUrl, role: p.role };
  });
}
...
function registerProductionRoutes(fastify: FastifyInstance, ctx: PluginContext) {
  ...
  fastify.post<{
    Body: { username: string; password: string; nickname: string; avatar?: string; turnstileToken?: string };
  }>('/auth/register', { preHandler: [registerLimiter], bodyLimit: 10 * 1024 * 1024 }, async (request, reply) => {
  ...
  fastify.post<{ Body: { username: string; password: string; turnstileToken?: string } }>(
    '/auth/login',
  ...
  fastify.post<{ Body: { username: string; password: string; githubId: string; githubAvatarUrl?: string } }>(
    '/auth/bind-github',
    { preHandler: [loginLimiter] },
    async (request, reply) => {
