...
packages/shared/src/rules/
  rule-registry.ts        # Rule 注册表（核心）
  rule-types.ts           # RuleDefinition 接口定义
  house-rules-engine.ts   # 引擎入口（遍历注册表执行规则）
  rules/                  # 内置村规实现（每个文件一条或一组相关规则）
    stacking.ts           # stackDrawTwo, stackDrawFour, crossStack
    deflection.ts         # reverseDeflectDrawTwo, reverseDeflectDrawFour, skipDeflect
    hand-swap.ts          # zeroRotateHands, sevenSwapHands
    draw-rules.ts         # drawUntilPlayable, forcedPlayAfterDraw, forcedPlay, deathDraw
    play-restrictions.ts  # noWildFinish, noFunctionCardFinish, misplayPenalty
    scoring.ts            # doubleScore, elimination, revengeMode
    modes.ts              # fastMode, blindDraw, silentUno, noHints, noChallengeWildFour
    multi-play.ts         # multiplePlaySameNumber, bombCard, jumpIn
    limits.ts             # handLimit, handRevealThreshold, blitzTimeLimit, unoPenaltyCount
    team.ts               # teamMode
```
